...
import com.eumji.zblog.util.ExceptionUtil;
...
class ExceptionHandlerController {
	...
	public ModelAndView notFound(HttpServletRequest request, NotFoundException exception){
		...
		logger.error("Request page: " + uri + " raised NotFoundException : " + exception, exception);
		...
		model.addObject("customMessage", ExceptionUtil.getExceptionInfo(exception));
		return model;
	}
	...
	public ModelAndView exception(HttpServletRequest request, Exception exception) {
		...
		model.addObject("path", uri);
		//model.addObject("customMessage", exception.getMessage());
		model.addObject("customMessage",org.apache.commons.lang.exception.ExceptionUtils.getFullStackTrace(exception)) ;
		return model;
	}
}